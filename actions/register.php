...
$username = $password = $confirm_password =  $invite ="";
$username_err = $password_err = $confirm_password_err = $invite_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty(trim($_POST["invite"])) || $username_err === "This username is already taken.") {
        ...
        $sql2 = "UPDATE invite SET ownedby = ? WHERE code = ?";
        if ($stmt = mysqli_prepare($link, $sql)) {
            //Bind variables to the prepared statement as parameters
            ...
            //Attempt to execute the perpared statement
            if (mysqli_stmt_execute($stmt)){
                //Store result
                mysqli_stmt_store_result($stmt);
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    mysqli_stmt_bind_result($stmt,$used);
                    ...
                    if ($used === 0)  {
                            ...
                            mysqli_stmt_bind_param($stmt, "ss", $param_username, $param_invite);
                            mysqli_stmt_execute($stmt);
                    } else {
    ...
    <div class="wrapper">
        ...
        <form id="formLogin" class="form" method="post">
            ...
            <div class="form-group <?php echo (!empty($invite_err)) ? 'has-error' : ''; ?>">
                ...
                <input type="password" name="invite" class="form-control" value="<?php echo $invite; ?>">
